...
public:
    ...
    static MyQueue * getInstance() { return _my_queue; }
}

// 饿汉模式，再我们调用 getInstance 之前，类里面的单例变量就创建出来了
MyQueue *MyQueue::_my_queue = new MyQueue;
```
...
public:
    ...
    static MyQueue * getInstance() {
        if (this->_my_queue == nullptr) {
            _my_queue = new MyQueue;
        }
        return _my_queue; 
    }
}
